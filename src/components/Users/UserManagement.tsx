...
export default function UserManagement() {
  ...
  const loadUsers = async () => {
    try {
      ...
      const response = await apiClient.get<UsersResponse>(
        `/users/admin/list/all?${params.toString()}`
      );
  ...
  const handleViewUser = async (userId: number) => {
    try {
      const user = await apiClient.get<User>(`/users/admin/${userId}`);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      // Map to backend camelCase field names
      const updateData: any = {};
      if (editForm.username) updateData.username = editForm.username;
      if (editForm.full_name) updateData.fullName = editForm.full_name; // Backend uses camelCase
      ...
      const response: any = await apiClient.put(`/users/admin/${selectedUser.user_id}`, updateData);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/users/admin/${selectedUser.user_id}`);
  ...
  const handleCreateUser = async () => {
    try {
      ...
      if (!editForm.username || !editForm.email || !editForm.password) {
        alert('Vui lòng điền đầy đủ thông tin bắt buộc: Tên đăng nhập, Email, Mật khẩu');
        return;
      }
      ...
      const newUserData: any = {
        ...
        fullName: editForm.full_name || editForm.username,
      };
      
      if (editForm.phone) newUserData.phone = editForm.phone;
      if (editForm.address) newUserData.address = editForm.address;
      
      console.log('Creating new user:', newUserData);
      
      const response: any = await apiClient.post('/users/register', newUserData);
